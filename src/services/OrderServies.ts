...
class OrderServies {
    ...
    getListCartById = async (user_id: string) => {
        ...
        if (user_id !== "") {
            if (checkOrderIsTemp.rows.length == 0) {
                await pool.query(`INSERT INTO public."order"(id_order, user_id, time_order, status_order, order_user_id, istemporary) VALUES('${idOrder}', '${user_id}', '${time_order}',false,'', false);`)
            } else {
    ...
    listCheckOut = async (user_id: string, pageSize: number, page: number) => {
        ...
        let totalQuantityProduct = await pool.query(`select id_order  from "order" where user_id = '${user_id}' and isTemporary = true group by id_order`)
    ...
    updateStatusOrder = async (id_order: string,email:string,nameUser:string) => {
        await pool.query(`UPDATE public."order" SET status_order=true WHERE id_order='${id_order}';`).then(()=>{
            ...
            var mailOptions = {
                ...
                to: `${email}`,
                subject: `Xác nhận thanh toán đơn hàng #${id_order} thành công từ ViettelStore`,
                html: `<h1>Welcome</h1><p>That was easy!</p>
                    <br/>
                    <p>Messenger:Cảm ơn anh/chị : ${nameUser} đã thanh toán đơn hàng thành công</p>
                `,
            };
